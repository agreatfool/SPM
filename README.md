...
* proto文件夹：项目自编写的protos文件
* spm_protos文件夹：用于存放包管理软件安装的protos文件   

每个微服务下都会有一个包管理配置文件：spm.json
...
### 版本冲突
由于版本依赖中可能存在版本冲突：即两个模块会同时依赖一个同一个模块的 的多个不同版本，所以在spm_protos文件夹下会有两种显示方法：

* 携带版本号的proto包，格式：模块名__v主版本号：
	* 肯定被spm_protos下的某个proto包依赖
	* 肯定存在同名不同版本的其他proto包
* 不携带版本号的proto包
	* 肯定直接被某个模块依赖
	* 可能存在同模块的不同版本 
