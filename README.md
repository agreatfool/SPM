...
> sasdn-pm delete -h
  ...
  Usage: sasdn-pm-delete <package> <password>
...
> sasdn-pm install -h
  ...
  Usage: sasdn-pm-install [<package>[@version]]
...
> sasdn-pm installed -h
  ...
  Usage: sasdn-pm-installed [package]

  show all installed protos or specified proto if package name provided
```
...
> sasdn-pm list -h
  ...
  show all remote protos registered in SPM
```
...
> sasdn-pm uninstall -h
  ...
  Usage: sasdn-pm-uninstall <package>
...
#### 2.3.2 重命名新包方案
* 考虑`2.3.1`中提到的`情况2`：真实生产环境下，依赖包（微服务）`m`的`2.x.x`和`1.x.x`两个版本都会在生产环境上共存
* 应用程序`a`依赖`1.x.x`版本的`m`，应用程序`b`依赖`a`
* 在`b`的开发过程中，需要安装`2.x.x`版本的`m`，且同时需要安装应用程序`a`（这里其实是作为一个依赖）
* `b`直接安装`m`会安装版本`2.x.x`
* 同事，`b`也需要安装`a`，但因为`a`依赖了`1.x.x`版本的`m`，因此实际上`b`也需要依赖到`1.x.x`版本的`m`
* 这种情况下，选择`rename`方案，此时，SPM会将后安装的`m`重命名为`m__v2`（某些安装顺序下可能生成`m__v1`）
* 这种方案下，某些安装包就会存在多个版本共存的情况

### 2.4 权限控制
权限控制分为仓库密钥(Repository Secret)控制和管理员密码(Admin Password)控制

#### 2.4.1 Repository Secret
仓库秘钥主要是限制仓库的发布以及后续的更新发布行为。每个仓库在发布前都必须申请一个仓库密钥，用来发布新版的 proto 包。具体流程如下：

* 创建微服务代码库
* 在代码库根目录下使用`sasdn-pm secret`申请该仓库的秘钥，秘钥会以文件的形式（./.spmlrc）保存在该仓库的根目录下
* 执行发布命令`sasdn-pm publish`会读取该仓库根目录下的秘钥文件，如果文件不存在则无法进行发布操作

#### 2.4.2 Admin Password
管理员密码是用来限制仓库删除操作的，只有拥有管理员密码的Admin才有权执行任何仓库的删除操作。普通程序员只能发布，而不能删除。删除流程如下：

* 管理员密码在初始化数据库表`spm_global_secret`时手动填入
* 删除操作需要执行`sasdn-pm delete`，在执行该命令的时候需要每次都手动输入密码
